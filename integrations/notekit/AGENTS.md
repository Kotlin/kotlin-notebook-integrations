### Notekit Project Requirements & Guidelines

This document summarizes the key architectural decisions, requirements, and technical details for the Notekit integration. This information is intended for other agents working on this project.

#### 1. Core Architecture

- **Modules & Project Structure**:
    - `notekit-api`: Core logic, protocol implementation, and the `Notekit` interface with its implementation.
    - `notekit-jupyter`: Integration with Kotlin Jupyter Notebook. Contains the Jupyter-specific integration code that registers `notekit` in the notebook scope.
    - `notekit-tests`: Integration and REPL tests.
- **Comm Target**:
    - `jupyter.notekit.v1`: Target for all notebook requests.
...
- **Location**: `JsonConversions.kt` in `notekit-api`.
- **Functions**:
...
- **REPL Tests**: Inherit from `AbstractNotekitReplTest`. Use `shouldHaveNextOpenEvent`, `shouldHaveNextMessageEvent`, `shouldHaveMethod`, `shouldHaveRequestId`, etc., to verify the sequence of Comm events.
...
After loading the integration with `%use notekit`, use the `notekit { }` DSL to access the API. All operations inside the block are suspend functions.
...
notekit { insertCell(index, newCell) }
notekit { appendCell(newCell) }
notekit { deleteCell(index) }
notekit { replaceCell(index, newCell) }
notekit { setNotebookMetadata(map, merge = true) }

// Execution operations
notekit { executeCell(0) }
notekit { executeCellRange(0, 3) }
notekit { executeAllCells() }
```

#### 8. Error Handling

- All operations throw `NotekitException` on failure.
