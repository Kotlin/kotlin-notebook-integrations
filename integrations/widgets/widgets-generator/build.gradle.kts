...
val generateWidgets by tasks.registering(JavaExec::class) {
    ...
    val apiOutputDir = project(projects.integrations.widgets.widgetsApi.path).file("src/generated/kotlin")
    val jupyterOutputDir = project(projects.integrations.widgets.widgetsJupyter.path).file("src/generated/kotlin")
        ...
        fun compareDirs(
            ...
            if (expectedRelativePaths != actualRelativePaths) {
                ...
                throw GradleException(
                    "Generated files mismatch.\n" +
                        ...
                        "Please run :integrations:widgets:widgets-generator:generateWidgets and commit the changes.",
                )
            }
            ...
            for (relativePath in expectedRelativePaths) {
                ...
                if (expectedFile.readText() != actualFile.readText()) {
                    throw GradleException(
                        "Generated file $relativePath differs from the expected one. Please run :integrations:widgets:widgets-generator:generateWidgets and commit the changes.",
                    )
                }
            }
        }
...
val checkWidgetsRegenerated by tasks.registering(CheckWidgetsRegenerated::class) {
    ...
    apiOutput.set(project(projects.integrations.widgets.widgetsApi.path).file("src/generated/kotlin"))
    jupyterOutput.set(project(projects.integrations.widgets.widgetsJupyter.path).file("src/generated/kotlin"))
