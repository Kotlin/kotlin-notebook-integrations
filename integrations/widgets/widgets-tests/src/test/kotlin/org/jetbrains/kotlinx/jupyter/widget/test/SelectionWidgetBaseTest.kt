...
class SelectionWidgetBaseTest : AbstractWidgetReplTest() {
    ...
    @Test
    fun `range selection labels as pair`() {
        execRaw("val s = selectionRangeSliderWidget()")
        execRaw("""s.optionsLabels = listOf("a", "b", "c", "d")""")
        execRaw("s.index = 1..2")
        execRaw("s.labels") shouldBe ("b" to "c")
        execRaw("s.value") shouldBe ("b" to "c")

        execRaw("""s.labels = "a" to "d"""")
        execRaw("s.index") shouldBe (0..3)
        execRaw("s.value") shouldBe ("a" to "d")

        execRaw("""s.value = "b" to "c"""")
        execRaw("s.index") shouldBe (1..2)
        execRaw("s.labels") shouldBe ("b" to "c")

        execRaw("s.labels = null")
        execRaw("s.index") shouldBe null
        execRaw("s.value") shouldBe null
    }
}
